...
class MultiBotRunner : CliktCommand() {
  ...
  override fun run() {
    ...
    val apis =
      apiFabric.createApis(initDatabase, TeacherResolverKind.RANDOM, config.botConfig.adminIds)

    runBlocking {
      launch {
        StudentRunner(studentBotToken, apis.studentApi, apis.parentApi, apis.errorManagementService)
          .run()
      }
      launch {
        while (true) {
