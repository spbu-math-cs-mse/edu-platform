...
fun DefaultBehaviourContextWithFSM<BotState>.strictlyOnStartState(studentStorage: StudentStorage) {
  strictlyOn<StartState> { state ->
    bot.sendSticker(state.context, Dialogues.greetingSticker)
    if (state.context.username == null) {
      return@strictlyOn null
    }
    val studentResolvedByTgId = studentStorage.resolveByTgId(state.context.id).get()?.id
    val studentId = studentResolvedByTgId ?: registerStudent(state.context, studentStorage)
...
private suspend fun BehaviourContext.registerStudent(
  ...
  val firstName = waitTextMessageWithUser(tgUser.id).first().content.text

  bot.send(tgUser, Dialogues.askLastName(firstName))
  val lastName = waitTextMessageWithUser(tgUser.id).first().content.text

  val askGradeMessage =
    bot.send(tgUser, Dialogues.askGrade(firstName, lastName), replyMarkup = Keyboards.askGrade())
