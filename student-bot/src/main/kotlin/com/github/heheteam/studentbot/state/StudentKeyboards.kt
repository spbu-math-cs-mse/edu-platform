...
object StudentKeyboards {
  ...
  const val DACHSHUND_QUEST = "dachshundQuest"
  ...
  fun menu(selectedCourseId: CourseId?) = inlineKeyboard {
    if (selectedCourseId != null) {
      inlineCourseMenu()
      row { dataButton("\uD83D\uDC36 Квест от Таксы Дуси", DACHSHUND_QUEST) }
    } else {
      inlineDachshundMenu()
    }
  ...
  private fun MatrixBuilder<InlineKeyboardButton>.inlineDachshundMenu() {
    row { dataButton("\uD83C\uDFAE Начать квест!", FREE_ACTIVITY) }
  ...
  fun dachshundMenu() = inlineKeyboard {
    inlineDachshundMenu()
