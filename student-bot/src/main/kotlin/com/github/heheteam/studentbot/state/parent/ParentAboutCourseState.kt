...
class ParentAboutCourseState(override val context: User, override val userId: ParentId) :
  ...
  private fun processKeyboardButtonPresses(
    ...
    val state =
      when (callback.data) {
        ParentKeyboards.ABOUT_TEACHERS -> ParentAboutTeachersState(context, userId)
        ParentKeyboards.METHODOLOGY -> ParentMethodologyState(context, userId)
        ParentKeyboards.COURSE_RESULTS -> ParentCourseResultsState(context, userId)
        ParentKeyboards.RETURN_BACK -> ParentMenuState(context, userId)
        else -> null
      }
